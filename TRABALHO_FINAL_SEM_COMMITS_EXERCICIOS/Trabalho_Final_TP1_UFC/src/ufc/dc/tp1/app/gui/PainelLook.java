...
public class PainelLook extends JPanel {
	...
	public PainelLook(PainelItem painelItem) {
    ...
    private void adicionarCartaoLook(Look look){
        ...
        if (look.getCaminhoImagem() != null && !look.getCaminhoImagem().isEmpty()) {
    ...
    private void mostrarDetalhesLook(Look look) {
        ...
        areaUsos.setFont(new Font("Arial", Font.PLAIN, 12));
        ...
        if (look.getHistoricoDeUsos() != null && look.getHistoricoDeUsos().isEmpty() == false) {
            for (UtilizacaoDeLook uso : look.getHistoricoDeUsos()) {
                areaUsos.append("Data: " + uso.getData().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) + "\n");
                areaUsos.append("Descrição: " + uso.getDescricao() + "\n");
                areaUsos.append("------------------------------\n");
            }
        } else {
