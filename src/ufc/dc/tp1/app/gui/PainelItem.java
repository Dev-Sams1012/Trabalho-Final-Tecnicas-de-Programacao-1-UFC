...
public class PainelItem extends JPanel {
	...
	private void deletaItem(Item item) {    	
		...
		for(ILavavel lavavel : listaItensSujos) {
			if (((Item)lavavel).getId().equals(item.getId())) {
				int indiceItemSujo = listaItensSujos.indexOf(lavavel);
				if (indiceItemSujo != -1) {
					listaItensSujos.remove(indiceItemSujo);
					break;
				}
			}
		}
	...
	private void atualizarPainelEstasticas() {
		...
		List<Item> menosUsados = new ArrayList<>();
        for(Item item : listaItens){
			if(menosUsados.isEmpty()) menosUsados.add(item);
			else{
				if(item.getQuantidadeUsos() < menosUsados.get(0).getQuantidadeUsos()){
					menosUsados.clear();
					menosUsados.add(item);
				} else if (item.getQuantidadeUsos() == menosUsados.get(0).getQuantidadeUsos()) {
					menosUsados.add(item);
				}
			}
		}

		List<Item> maisUsados = new ArrayList<>();
		for(Item item : listaItens){
			if(maisUsados.isEmpty()) maisUsados.add(item);
			else{
				if(item.getQuantidadeUsos() > maisUsados.get(0).getQuantidadeUsos()){
					maisUsados.clear();
					maisUsados.add(item);
				} else if (item.getQuantidadeUsos() == maisUsados.get(0).getQuantidadeUsos()) {
					maisUsados.add(item);
				}
			}
		}
		...
		textoEstatistica += "Item(s) menos usado(s): \n";

		for(Item item : menosUsados){
			textoEstatistica += item.getId() + " - Usos: " + item.getQuantidadeUsos() + "\n";
		}

		textoEstatistica += "-----------------\n";

		textoEstatistica += "Item(s) mais usado(s): \n";
